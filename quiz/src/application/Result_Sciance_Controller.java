...
public class Result_Sciance_Controller {
	...
	private void initialize(){
	    ...
	    String updateSql = "UPDATE useraccount SET Sci_score = ? WHERE Username = ?;";
	    
	    try(PreparedStatement preparedStatement = connectDB.prepareStatement(updateSql)){
	    	preparedStatement.setInt(1, Sciance_controller.Correct);
		    preparedStatement.setString(2, username);
		    preparedStatement.execute();
	    }catch(Exception e) {
	    	e.printStackTrace();
	    }
		Grade_text.setText(String.valueOf(Sciance_controller.Correct) + "/10");
		score.setText("      " +String.valueOf(Sciance_controller.Correct)+ "/10");
		
		float correctf = (float)Sciance_controller.Correct/10;
		Correct.setProgress(correctf);
		
		int correct = Sciance_controller.Correct;
		if (correct <= 7) {
