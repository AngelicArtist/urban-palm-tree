...
public class Result_Math_Controller {
	...
	private void initialize(){
	    ...
	    String updateSql = "UPDATE useraccount SET Math_Score = ? WHERE Username = ?;";
	    
	    try(PreparedStatement preparedStatement = connectDB.prepareStatement(updateSql)){
	    	preparedStatement.setInt(1, Math_Controller.Correct);
		    preparedStatement.setString(2, username);
		    preparedStatement.execute();
	    }catch(Exception e) {
	    	e.printStackTrace();
	    }
		Grade_text.setText(String.valueOf(Math_Controller.Correct) + "/10");
		score.setText("      " +String.valueOf(Math_Controller.Correct)+ "/10");
		
		float correctf = (float)Math_Controller.Correct/10;
		Correct.setProgress(correctf);
		
		int correct = Math_Controller.Correct;
		if (correct <= 7) {
